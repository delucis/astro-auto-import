...
function formatTypeDeclaration(name: string, exportName: string, module: string) {
  return `const ${name}: typeof import('${module}')['${exportName}']`;
}
...
function processImportsConfig(config: ImportsConfig) {
  ...
  for (const option of config) {
    if (typeof option === 'string') {
      const module = resolveModulePath(option);
      const name = getDefaultImportName(option);
      imports.push(formatImport(name, resolveModulePath(option)));
      exposures.push(formatExposure(name));
      declarations.push(formatTypeDeclaration(name, 'default', module));
    } else {
      for (const path in option) {
        const namedImports = option[path];
        const module = resolveModulePath(path);
        const [importString, exposureArray] = formatNamedImports(namedImports);
        imports.push(formatImport(importString, module));
        exposures.push(...exposureArray);
        declarations.push(
          ...namedImports.map((n) => {
            const [from, as] = Array.isArray(n) ? n : [n, n];
            return formatTypeDeclaration(as, from, module);
          })
        );
      }
    }
  ...
  return { imports, exposures, declarations };
}
...
/** Generate the contents of a `.d.ts` file declaring auto-imports globally. */
function generateDTS(config: ImportsConfig) {
  const { declarations } = processImportsConfig(config);
  return `// Generated by 'astro-auto-import'
declare global {
  ${declarations.join('\n  ')}
}
export {}`;
  ...
  return {
    ...
    hooks: {
      'astro:config:setup': ({ injectScript, config, updateConfig }) => {
        ...
        const dtsPath = new URL(import.meta.url.replace(/index\.js$/, 'global.d.ts'));
        const declarations = generateDTS(integrationConfig.imports);
        writeFileSync(dtsPath, declarations, 'utf-8');
      },
    },
  };
}
